...
- (void)setupGenerateQRCodeWithInfo:(NSString *)info {
    ...
    imageView.image = [SGQRCodeTool SG_generateWithDefaultQRCodeData:info imageViewWidth:imageViewW];
...
#pragma mark - - - 模仿支付宝二维码样式（添加用户头像）
    ...
    borderView.layer.contents = (id)[UIImage imageNamed:@"bg.jpg"].CGImage;
...
- (void)setupGenerate_Icon_QRCodeWithInfo:(NSString *)info Picture:(NSString *)picture {
    ...
    imageView.image = [SGQRCodeTool SG_generateWithLogoQRCodeData:info logoImageName:picture logoScaleToSuperView:scale];
